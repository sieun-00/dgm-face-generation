...
\paragraph{Random seeds (final submission).} Image $i$ is generated from integer seed $i$ for
$i = 0, \dots, 999$, so the full seed set is $\{0, 1, \dots, 999\}$. Each seed initializes a NumPy
\texttt{RandomState} that draws the latent $z_i \in \mathbb{R}^{512}$, which the generator maps under
truncation $\psi = 1.0$ and \texttt{noise\_mode=const}. Generation is therefore deterministic: the same
seed set and checkpoint reproduce the submitted images (identical up to GPU floating-point noise). The
seed set and $\psi$ are also recorded in \texttt{config.py}.
