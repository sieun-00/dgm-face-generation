% DGM Spring 2026 — Face Generation Challenge — Final Report (NeurIPS style)
% Set the title/author/GitHub URL. An optional sample grid can be added at the
% commented figure in Section 5.
\documentclass{article}

\usepackage[final]{neurips_2023}   % match your template's .sty filename/year
...
\title{Fine-tuning StyleGAN2-ADA on CelebV-HQ for the Face Generation Challenge}

\author{%
  Firstname~Lastname \\
  Department of XXX, UNIST \\
  \texttt{name@unist.ac.kr} \\
}
...
\texttt{requirements.txt} and a \texttt{Dockerfile}, and the seeds and $\psi$ live in \texttt{config.py}.
